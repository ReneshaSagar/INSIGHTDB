...
async function fetchTableSchema(tableName) {
    ...
    tableSchema.columns.forEach(col => {
        ...
        tr.innerHTML = `
            <td>${col.name} ${tableSchema.potential_keys.includes(col.name) ? '<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align: middle; margin-left: 6px; color: var(--primary);"><path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"></path></svg>' : ''}</td>
            <td>${col.type}</td>
            <td>-</td> 
            <td>${nullPct}%</td>
            <td>${uniquePct}%</td>
...
function setupEventListeners() {
    ...
    function applyTheme(theme) {
        if (theme === 'dark') {
            ...
            if (lightBtn && darkBtn) {
                // Dark Active styling overrides
                darkBtn.style.background = '#1f2937';
                darkBtn.style.color = '#3b82f6';
                darkBtn.style.boxShadow = '0 1px 3px rgba(0,0,0,0.5)';
                lightBtn.style.background = 'transparent';
                lightBtn.style.color = '#94a3b8';
            ...
            if (lightBtn && darkBtn) {
                // Light Active styling overrides
                lightBtn.style.background = '#ffffff';
                lightBtn.style.color = 'var(--primary)';
                lightBtn.style.boxShadow = '0 1px 3px rgba(0,0,0,0.1)';
